...
from utils import validate, init_weights, save_prediction,get_embedings
...
def main(args, seed):
    ...
    if args.dataset.startswith("finetune"):
        ...
        X_train,y_train = get_embedings(
                args, model, train_loaders['train_loader'])
        print(f"{X_train.shape=}, {y_train.shape=}")
        X_val,y_val = get_embedings(
                args, model, valid_loader)
        print(f"{X_val.shape=}, {y_val.shape=}")
        X_test,y_test = get_embedings(
    ...
    if args.dataset.startswith("finetune"):
        ...
        if not os.path.exists(args.model_path):
            ...
            if os.path.exists(new_yaml_path):
                os.remove(new_yaml_path) # Usuń istniejący plik docelowy przed zmianą nazwy
