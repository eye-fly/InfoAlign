...
__all__ = ["FingerprintDecoder", "GEDecoder", "CPDecoder", "SMILESDecoder"]


class FingerprintDecoder(nn.Module):
    """Mean-pool encoder tokens → MLP → reconstruct 2048-bit Morgan fingerprint."""
    def __init__(self, d, hidden=512, out_dim=2048):
        super().__init__()
    ...
    def forward(self, z, pad_mask=None):
        # z: (B, L, d) → masked mean pool over sequence → (B, d) → (B, out_dim)
...
class VectorDecoder(nn.Module):
    """Generic decoder for vector data"""
    ...
    def forward(self, z):
        # z: (B, L, d) → mean pool → (B, d) → (B, out_dim)
        return self.net(z.mean(dim=1))

    def loss(self, z, targets):
        valid = ~torch.isnan(targets).any(dim=1)  # (B,) — only compounds with data
        if not valid.any():
            return torch.tensor(0.0, device=z.device)
        pred = self.forward(z[valid])
        return F.mse_loss(pred, targets[valid])


class GEDecoder(VectorDecoder):
